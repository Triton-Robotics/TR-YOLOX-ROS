...
void YoloXNode::colorImageCallback(const sensor_msgs::msg::Image::ConstSharedPtr &ptr) {
    ...
    if (this->params_.use_bbox_ex_msgs) {
        ...
        if (!detections.detections.empty()) {
            ...
            this->sharedDetWriter_->writeDetWithImage(img->image, shared_detections, timeGrabbed);
        } else {
    ...
    RCLCPP_INFO(this->get_logger(), "Inference time: %5ld us Non Inference time: %5ld",
                elapsed.count(), elapsed_noninf.count() - elapsed.count());
}

bboxes_ex_msgs::msg::BoundingBoxes
YoloXNode::objects_to_bboxes(const cv::Mat &frame, const std::vector<yolox_cpp::Object> &objects,
