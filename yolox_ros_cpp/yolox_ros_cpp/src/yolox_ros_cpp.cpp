...
void YoloXNode::onInit() {
    ...
    if (this->params_.use_shared_memory) {
        ...
        this->sharedImageReader_ = std::make_unique<SharedImageReader>(
            "camera_image", 1200, 1920, 3, "CV_8U");
        ...
        this->sub_image_ = image_transport::create_subscription(
            ...
            std::bind(&YoloXNode::colorImageCallback, this,
                      std::placeholders::_1),
            "raw", qos_profile.get_rmw_qos_profile(), *this->sub_options_);

        RCLCPP_INFO(this->get_logger(),
                    "Using ROS topic input - subscribed to %s",
                    this->params_.src_image_topic_name.c_str());
    }
}
...
void YoloXNode::colorImageCallback(
    ...
    // RCLCPP_INFO(this->get_logger(), "Ms %0.5f",
    // yolox_ipc_latency.nanoseconds() / 1000000.0F);
    ...
    auto objects = this->yolox_->inference(img->image, this->d_image_,
                                           this->d_output_, this->stream_);
    ...
    if (!detections.detections.empty()) {
        ...
        // ptr detections_msg.detection_info.detections = detections.detections;
...
void YoloXNode::sharedMemoryImageCallback() {
    ...
    if (!this->sharedImageReader_ ||
        !this->sharedImageReader_->isInitialized()) {
        return;
    }
    ...
    // Run YOLOX inference
    auto objects = this->yolox_->inference(image, this->d_image_,
                                           this->d_output_, this->stream_);
    auto end = std::chrono::system_clock::now();
    auto elapsed =
        std::chrono::duration_cast<std::chrono::microseconds>(end - now);
    ...
    if (!detections.detections.empty()) {
        ...
        RCLCPP_DEBUG(this->get_logger(),
                     "No detections from shared memory frame %d",
                     current_frame);
    }
    std::chrono::system_clock::time_point end_yolo =
